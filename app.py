...
@st.cache_data
def load_all():
    d1 = pd.read_csv("hr_attrition_clean.csv")
    d1['Dataset'] = 'IBM-Style Corp (2016)'
    d1['RemoteWork'] = 'On-site'
    d1['StockOptions'] = 0

    d2 = pd.read_csv("hr_attrition_clean_v2.csv")
    d2['Dataset'] = 'IBM-Style Corp Fixed (2016)'
    d2['RemoteWork'] = 'On-site'
    d2['StockOptions'] = 0

    d3 = pd.read_csv("hr_techco_2024.csv")
    d3['Dataset'] = 'Tech Co (2024)'

    return d1, d2, d3, pd.concat([d1, d2, d3], ignore_index=True)

d1, d2, d3, all_data = load_all()

DATASET_META = {
    'IBM-Style Corp (2016)':       {'color': '#378ADD', 'employees': len(d1), 'note': 'Traditional manufacturing/services firm. On-site only. No remote work or stock options.'},
    'IBM-Style Corp Fixed (2016)': {'color': '#BA7517', 'employees': len(d2), 'note': 'Same company — corrected data where work-life balance was improperly coded.'},
    'Tech Co (2024)':              {'color': '#1D9E75', 'employees': len(d3), 'note': '2024 tech startup. Hybrid/remote workforce. Stock options included. Younger, higher-paid.'},
}

DATASETS = {
    'IBM-Style Corp (2016)': d1,
    'IBM-Style Corp Fixed (2016)': d2,
    'Tech Co (2024)': d3,
}
...
st.sidebar.markdown("---")
page = st.sidebar.radio("Navigate", ["📊 Dashboard", "📊 Compare Datasets", "🌍 Benchmarks", "📄 Resume — HR", "📄 Resume — Analyst"])
st.sidebar.markdown("---")
...
# ════════════════════════════════════════════════════════
    ...
    bench = pd.DataFrame({
        'Source': ['IBM-Style Corp (2016)', 'IBM-Style Corp Fixed (2016)', 'Tech Co (2024)',
        ...
        'Rate (%)': [
            ...
            round(d3['AttritionBinary'].mean()*100, 1),
            20.0, 23.0, 10.0
        ],
        'Type': ['Our Data','Our Data','Our Data','Benchmark','Benchmark','Target']
    })
    ...
    gallup = pd.DataFrame({
        ...
        'Percentage': [21, 62, 15, round((d2['JobSatisfaction']>=3).mean()*100, 0)],
...
# ════════════════════════════════════════════════════════
    ...
    st.title("📄 Resume — HR Version")
    st.caption("This version is written for HR, People & Culture, and Management roles.")

    hr_text = """HR Attrition Analytics Dashboard
Tools: Python (pandas, matplotlib, seaborn) · Streamlit · Plotly

• Analysed 3 company datasets (4,090+ employee records) to identify key drivers
  of workforce attrition across department, age, income, overtime, and job satisfaction
• Identified overtime as the strongest universal predictor — overtime workers showed
  3× higher attrition across all three company profiles
• Benchmarked findings against Mercer 2024 and Gallup 2025 global reports,
  contextualising attrition rates within industry norms
• Built an interactive 4-page dashboard with company selector, cross-dataset
  comparisons, and department/gender filters for HR decision-makers
• Surfaced actionable recommendation: highest-risk segment is employees under 25
  with low satisfaction scores, recommending targeted early-career engagement programs"""
    ...
    """, unsafe_allow_html=True)
    st.code(hr_text, language=None)
    st.download_button("⬇️ Download HR Resume Version (.txt)", hr_text,
                       file_name="resume_hr_version.txt", mime="text/plain")
    st.markdown("**Skills demonstrated:**")
    for tag in ["People Analytics","Attrition Modelling","Benchmark Research","Dashboard Design","Python","Streamlit","HR Strategy","Data Storytelling"]:
        st.markdown(f'<span class="tag">{tag}</span>', unsafe_allow_html=True)

# ════════════════════════════════════════════════════════
# PAGE: RESUME — ANALYST
# ════════════════════════════════════════════════════════
elif page == "📄 Resume — Analyst":
    st.title("📄 Resume — Analyst Version")
    st.caption("This version is written for Data Analyst, Business Analyst, and People Analytics roles.")

    analyst_text = """HR Attrition Analytics Dashboard
Tools: Python (pandas, matplotlib, seaborn) · Streamlit · Plotly

• Designed and executed end-to-end data analysis pipeline across 3 company datasets
  (4,090+ employee records, 19 variables each)
• Performed EDA to surface attrition patterns by department, tenure, income band,
  overtime status, satisfaction score, remote work type, and stock option level
• Applied data validation to detect and correct a synthetic data flaw where
  work-life balance showed an inverted attrition relationship — rebuilt generation
  logic with explicit probability mapping per variable
• Cross-referenced findings with Mercer 2024, Gallup 2025, SHRM, and BambooHR
  benchmarks to contextualise internal metrics against global standards
• Modelled estimated annual cost of attrition using Gallup and SHRM cost-per-departure
  formulas, translating HR metrics into a direct business cost
• Deployed interactive 4-page dashboard on Streamlit Cloud with dataset selector,
  cross-company comparison charts, and real-time filters"""
    ...
    """, unsafe_allow_html=True)
    st.code(analyst_text, language=None)
    st.download_button("⬇️ Download Analyst Resume Version (.txt)", analyst_text,
                       file_name="resume_analyst_version.txt", mime="text/plain")
    st.markdown("**Skills demonstrated:**")
    for tag in ["EDA","Data Validation","Benchmark Analysis","Pipeline Design","Python","Plotly","Streamlit","Statistical Analysis","Business Framing","Cost Modelling"]:
        st.markdown(f'<span class="tag">{tag}</span>', unsafe_allow_html=True)
